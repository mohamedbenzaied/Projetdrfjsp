...
class job_shop_env():
    ...
    def __init__(self):
        self.job_cluster = self.expert_job.shape[1]#number of job_cluster(categories)
        self.expert = self.expert_job.shape[0]#number of expert  (machine)
        self.job_num = self.job.shape[0]#nombre des jobs
        self.process_time = self.expert_job#matrix process time
        self.expert_status = np.repeat(0,self.expert) ## how many jobs an expert is processing(notre cas 1)
        self.expert_process_job = [[] for i in range(self.expert)]#jobs processing by expert(chaque expert admet une machine)  
        self.expert_process_time = [[] for i in range(self.expert)]#process time de chaque job  
        self.job_waiting_time = [[] for i in range(self.expert)]#job waiting time
        self.left_job = self.job.shape[0]#number of jobs incomplete
        self.done = False#episode start/finish (true(finished episode))
        self.total_time = 0  ## total process time
        self.job_distribute_time = np.repeat(0,self.job.shape[0])#temps d'attente de job
        ...
        self.state = np.vstack((self.job_status,self.job_distribute_time))#This variable contains the state (current/next) given by the environment
    ...
    """The reset does not have arguments and return variables
and a state corresponding to the start of a new episode.

correspondant au début d'un nouvel épisode. Lorsque l'environnement est initié pour la première
ou lorsqu'un nouvel épisode commence, l'état de l'environnement et ses variables sont réinitialisés par cette fonction.
"""    
    def reset(self):
    ...
    """In this step function, the input is an action, and it returns a set of
elements: state, reward, and done
The reset function takes no arguments and returns variables and a state.

à partir duquel l'agent apprendra comment obtenir la plus grande récompense possible
étant donné un certain état initial, en effectuant une certaine action. Dans cette fonction, on établit
    ...
    def step(self, action):
        ...
        for i in range(self.expert):
            ...
            if job_id[i] in self.job_index:
                ...
                if action[i] == 0 or self.expert_status[i] == 1:
                    pass
                else:
